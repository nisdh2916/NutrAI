...
class FoodAddScreen extends StatefulWidget {
  ...
  const FoodAddScreen(
      {super.key, this.initialMealLabel = '아침', this.initialDate});
...
class _FoodAddScreenState extends State<FoodAddScreen> {
  ...
  void _onSave() async {
    ...
    final validFoods = _detectedFoods
        .where(
    ...
    if (validFoods.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('영양정보가 없는 음식만 있어요. 음식을 검색해서 추가해주세요.'),
            behavior: SnackBarBehavior.floating),
      );
      return;
    }
    ...
    try {
      ...
      if (mealType != 'snack') {
        ...
        if (exists) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content:
                  Text('$_mealLabel 식단이 이미 기록되어 있어요. 기존 기록을 삭제 후 다시 추가해주세요.'),
              behavior: SnackBarBehavior.floating,
            ),
          );
          return;
      ...
      for (final f in validFoods) {
        if (merged.containsKey(f.name)) {
          merged[f.name] =
              (food: merged[f.name]!.food, count: merged[f.name]!.count + 1.0);
        } else {
      ...
      for (final entry in merged.values) {
        final fid = await appState.getOrCreateFood(
          ...
          kcal: entry.food.kcal.toDouble(),
          carbG: entry.food.carb.toDouble(),
          proteinG: entry.food.protein.toDouble(),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('저장 실패: $e'), behavior: SnackBarBehavior.floating),
      );
    }
  }
        ...
        Expanded(
          child: CustomScrollView(slivers: [
            ...
            if (_state == _ScreenState.initial) ...[
              SliverToBoxAdapter(
                  child: _PhotoButtons(
                      onCamera: _onCamera, onGallery: _onGallery)),
              SliverToBoxAdapter(
      ...
      child: Row(children: [
        ...
        Expanded(
            child: _PhotoBtn(
                ...
                label: '갤러리에서 선택',
                sub: '사진 불러오기',
                bg: AppColors.carbSoft,
                iconBg: AppColors.carb,
                textColor: AppColors.carb,
                onTap: onGallery)),
      ]),
    );
  }
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(
          ...
          left: BorderSide(
              color: isEditing ? AppColors.brand : Colors.transparent,
              width: 3),
        ),
      ),
      child: Row(children: [
        ...
        Expanded(
          ...
          const SizedBox(height: 2),
          Text(
              '${food.kcal.round()}kcal · 탄 ${food.carb.round()}g · 단 ${food.protein.round()}g · 지 ${food.fat.round()}g',
        ...
        Padding(
          ...
          child: Column(
              children: searchResults
                  .take(4)
                  .map((f) =>
                      _SearchResultTile(food: f, onTap: () => onSelectAlt(f)))
                  .toList()),
        ),
