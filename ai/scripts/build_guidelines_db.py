...
실행:
  .venv\Scripts\python ai\scripts\build_guidelines_db.py
"""
import sys
import uuid
...
def load_guideline_docs(cfg: dict) -> tuple[list[str], list[dict], list[str]]:
    ...
    for i, chunk in enumerate(chunks):
        doc_id = f"guide_{cfg['condition']}_{i:04d}"
        meta = {
            "category":        "가이드라인",
            "source":          cfg["source"],
...
def main() -> None:
    ...
    for cfg in GUIDELINE_FILES:
        ...
        # 기존 가이드라인 문서 삭제 후 재삽입
        existing = collection.get(
            where={"$and": [{"category": {"$eq": "가이드라인"}},
                            {"condition": {"$eq": cond}}]},
            include=[],
        )
        if existing["ids"]:
