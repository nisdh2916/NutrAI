...
기존 음식 DB에 프랜차이즈 메뉴 데이터를 보강한다.

실행:
  python -m ai.scripts.db.add_franchise_data
"""
import csv
import sys
...
def build_document(row: dict) -> str:
    ...
    if kcal is not None:        parts.append(f"칼로리 {kcal}kcal")
    if carb is not None:        parts.append(f"탄수화물 {carb}g")
    if protein is not None:     parts.append(f"단백질 {protein}g")
    if fat is not None:         parts.append(f"지방 {fat}g")
    if sodium is not None:      parts.append(f"나트륨 {sodium}mg")
    if sugar is not None:       parts.append(f"당류 {sugar}g")
    if sat_fat is not None:     parts.append(f"포화지방산 {sat_fat}g")
    if cholesterol is not None: parts.append(f"콜레스테롤 {cholesterol}mg")
...
def main():
    ...
    if not CSV_PATH.exists():
        ...
        print("data/nutrition_db/ 에 식품의약품안전처 가공식품 CSV를 넣어주세요.")
        return
