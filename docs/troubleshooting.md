...
## 20. SQLite 숫자 컬럼 `as double?` 타입 캐스트 크래시

**증상:** 사용자 프로필 로드 시 `Lost connection to device` 또는 `TypeError: type 'int' is not a subtype of type 'double?'`

**원인:** SQLite는 소수점 없는 REAL 값을 `int`로 반환함. Dart에서 `as double?` 직접 캐스트 시 런타임 TypeError 발생.
...
**관련 파일:** `app/lib/models/db_models.dart` → `UserProfileEntity.fromMap()`

---

## 21. database_helper에 local_user_allergy 테이블 및 photo_path 컬럼 누락

**증상:** 앱 실행 시 `table meal has no column named photo_path` 또는 `no such table: local_user_allergy`

**원인:** SQL 스키마에는 정의되어 있으나 `database_helper.dart`의 `_onCreate()`에 누락. `MealEntity.toMap()`이 `photo_path`를 포함하는데 테이블에 컬럼이 없어 insert 실패.

**해결:**
- `_dbVersion` 2 → 3 으로 올림
- `_onCreate()`에 `meal` 테이블에 `photo_path TEXT` 추가
- `_onCreate()`에 `local_user_allergy` 테이블 생성 및 인덱스 추가
- `_onUpgrade()`에 `oldV < 3` 마이그레이션 블록 추가

**관련 파일:** `app/lib/database/database_helper.dart` → `_onCreate()`, `_onUpgrade()`

---

## 22. AllergyRepository에서 ConflictAlgorithm 미정의 에러

**증상:** `allergy_repository.dart` 컴파일 에러 — `Undefined name 'ConflictAlgorithm'`

**원인:** `ConflictAlgorithm`은 `sqflite` 패키지에 정의되어 있는데 import 누락
...
**관련 파일:** `app/lib/repositories/allergy_repository.dart`
